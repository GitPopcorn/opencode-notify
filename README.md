...
- **Double-ESC showed NETWORK INTERRUPTED on top of STOPPED BY YOU**: a real double-ESC surfaces as `{"name":"MessageAbortedError","data":{"message":"Aborted"}}`. Its name is "aborted"+"error", which the old `name.includes("aborterror")` check missed, so the bare "Aborted" text hit the `NETWORK_ERROR_HINTS` "aborted" entry and fired a NETWORK toast. The abort-name match is now the broader `name.includes("abort")` (still guarded: a message carrying a genuine connection signature stays a network interruption). A network/http error arriving within **5s after a user-cancel for the same session** is additionally treated as that abort's teardown and suppressed (log `L2003`).
