...
- **Double-ESC showed NETWORK INTERRUPTED on top of STOPPED BY YOU**: interrupting a run surfaces **two** `session.error`s — first `"This operation was aborted"` (correctly → STOPPED BY YOU), then `"fetch failed: read ECONNRESET"` when the SDK tears down the in-flight request the user just cancelled. The second one matched the network signature and fired a scary NETWORK toast. A network/http error arriving within **5s after a user-cancel for the same session** is now treated as that abort's teardown and suppressed (log `L2003`); a real network drop without a prior stop still fires normally.
